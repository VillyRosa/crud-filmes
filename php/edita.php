...
$id = limpaPost($_GET['key']);

echo var_dump($_FILES['poster']);
...
if ($_FILES['poster']['name'] != '') {
    ...
    $poster = $novoNome;
    $sql = $pdo->prepare("UPDATE filmes SET titulo=?, genero=?, diretor=?, categoria=?, descricao=?, poster=? WHERE id=?");
    $sql->execute([$titulo,$genero,$diretor,$categoria,$descricao,$poster,$id]);
} else {
    $sql = $pdo->prepare("UPDATE filmes SET titulo=?, genero=?, diretor=?, categoria=?, descricao=? WHERE id=?");
    $sql->execute([$titulo,$genero,$diretor,$categoria,$descricao,$id]);
}
